...
Ext.define("Sch.mixin.SchedulerView", {
    ...
    _initializeSchedulerView: function () {
        ...
        //               事件点击        鼠标点击         松开鼠标          双击事件        事件文本菜单        事件鼠标输入       鼠标事件         调整大小事件       调整大小开始事件     部分调整大小事件      在调整大小事件完成前        调整结束事件       拖动前事件         拖动开始事件       在下落事件完成前         下落事件      下落事件后        创建拖动前          拖动创建开始          拖动创建完成           拖动创建结束       拖动创建后         添加事件前        进度点击         双击表                日程表菜单
        var b = this;
        if (!this.eventPrefix) {
